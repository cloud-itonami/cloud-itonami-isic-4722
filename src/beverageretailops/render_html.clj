(ns beverageretailops.render-html
  ...
  (`beverageretailops.operation` -> `beverageretailops.governor` ->
  `beverageretailops.store`) through a scenario adapted from this repo's
  own `beverageretailops.sim` demo driver (`clojure -M:dev:run`, confirmed
  BEFORE writing this file to produce a sensible ledger against the real
  seeded store ids `store-1`..`store-3` -- ids that DO match
  `beverageretailops.store/demo-data`, so it was safe to reuse rather than
  ...
  (:require [jp-go-dds.skin]
            ...
            [beverageretailops.advisor :as advisor]
            [beverageretailops.store :as store]
            [beverageretailops.operation :as op]
            [langgraph.graph :as g]))

(def ^:private coordinator
  {:actor-id "coord-1" :actor-role :beverage-retail-coordinator :phase 3})

(defn- exec! [actor tid request]
  (g/run* actor {:request request :context coordinator} {:thread-id tid}))

(defn- approve! [actor tid]
  (g/run* actor {:approval {:status :approved :by "beverage-retail-coordinator-1"}}
...
(defn run-demo!
  ...
  (ALWAYS escalates -- approved), and a compliance-concern flag (ALWAYS
  ...
  a sales-record proposal that drifts into age-verification-override /
  responsible-service scope HARD-holds permanently. Every HARD hold never
  ...
  (let [db (store/seed-db)
    ...
    (exec! actor "t1-sales" {:op :log-sales-record :store-id "store-1"
                             :patch {:units-sold 24 :item "cabernet sauvignon" :units-returned 1}})

    (exec! actor "t2-staff" {:op :schedule-staffing-operation :store-id "store-1"
                             :patch {:shift "evening-counter" :date "2026-07-20"
                                     :window "17:00-21:00"}})

    (exec! actor "t3-supply-low" {:op :coordinate-supply-order :store-id "store-1"
                                  :patch {:item "wine bottle labels" :quantity 500
                                          :estimated-cost 85.0}})

    (exec! actor "t4-supply-high" {:op :coordinate-supply-order :store-id "store-1"
                                   :patch {:item "premium spirits restock" :quantity 1
                                           :estimated-cost 3200.0}})
    (approve! actor "t4-supply-high")

    (exec! actor "t5-flag" {:op :flag-compliance-concern :store-id "store-1"
                            :patch {:concern "customer at register 2 declined to present ID after cashier request, suspected underage purchase attempt"
                                    :confidence 0.92}})
...
(defn- verified-cell [{:keys [registered? verified?]}]
  (cond
    ...
    registered? "<span class=\"warn\">registered, liquor license pending</span>"
...
(def ^:private action-gate-rows
  ...
  ;; (README `Features`, `beverageretailops.governor`/`beverageretailops.phase`) --
  ...
  ["        <tr><td><code>:log-sales-record</code></td><td><span class=\"ok\">phase-3 auto-commit when clean &amp; high confidence</span></td></tr>"
   ...
   "        <tr><td><code>:flag-compliance-concern</code></td><td><span class=\"warn\">ALWAYS human approval &middot; never auto at any phase &middot; surfaces concern only, never finalizes age-verification or responsible-service</span></td></tr>"])
...
(defn render
  ...
  (let [ledger (vec (store/ledger db))
    ...
    (str
     "<html><head><meta charset=\"utf-8\"><title>cloud-itonami-isic-4722 &middot; specialized-beverage-retail</title><style>"
     (jp-go-dds.skin/dds+skin)
     "</style></head><body>\n"
     "<header class=\"bar\">\n"
     "  <h1>Retail sale of beverages in specialized stores (ISIC 4722) — Operator Console</h1>\n"
     "  <span class=\"badge\">read-only sample · governor-gated · compliance concern / high-cost supply always human-approved · HARD holds permanent</span>\n"
     "</header>\n"
     "<main>\n"
     "  <section class=\"card\">\n"
     "    <h2>Specialized beverage retail stores</h2>\n"
     "    <p class=\"muted\">Demo snapshot — build-time-generated from <code>beverageretailops.store</code> via <code>beverageretailops.render-html</code> (<code>clojure -M:dev:render-html</code>), regenerated from the real actor stack.</p>\n"
     "    <table>\n"
     "      <thead><tr><th>Store</th><th>Name</th><th>Kind</th><th>Registration / liquor license</th><th>Last op status</th></tr></thead>\n"
     "      <tbody>\n"
     store-rows "\n"
     "      </tbody>\n"
     "    </table>\n"
     "  </section>\n"
     "  <section class=\"card\">\n"
     "    <h2>Action gate (Beverage Retail Governor)</h2>\n"
     "    <p class=\"muted\">HARD holds cannot be overridden: store unverified (registration + liquor retail license), effect not <code>:propose</code>, and permanently out-of-scope age-verification-override / responsible-service finalization / ID-scanner actuation / alcohol-licensing-authority territory. An unregistered or unverified store never reaches a human.</p>\n"
     "    <table>\n"
