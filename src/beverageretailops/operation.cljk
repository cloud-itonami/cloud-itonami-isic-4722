(ns beverageretailops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (BeverageRetailAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the BeverageRetailGovernor (:govern)
  ...
  human beverage-retail operations coordinator. The approver resumes
  with `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [beverageretailops.advisor :as advisor]
            [beverageretailops.governor :as governor]
            [beverageretailops.phase :as phase]
...
(defn build
  ...
  `beverageretailops.store/Store`).
  opts:
    :advisor      -- a `beverageretailops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; BeverageRetailAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; BeverageRetailGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
