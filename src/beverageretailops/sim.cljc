(ns beverageretailops.sim
  ...
  phase), then a compliance-concern flag (ALWAYS escalates, at any
  ...
  has drifted into the permanently-excluded age-verification-override/
  responsible-service-decision scope."
...
(defn- approve! [actor tid]
  (g/run* actor {:approval {:status :approved :by "beverage-retail-coordinator-1"}} {:thread-id tid :resume? true}))

(defn -main [& _]
  (let [db (store/seed-db)
        coordinator-phase-1 {:actor-id "coord-1" :actor-role :beverage-retail-coordinator :phase 1}
        coordinator-phase-3 {:actor-id "coord-1" :actor-role :beverage-retail-coordinator :phase 3}
        actor (op/build db)]
    ...
    (let [r (exec-op actor "t1" {:op :log-sales-record :store-id "store-1"
                                  :patch {:units-sold 24 :item "cabernet sauvignon" :units-returned 1}} coordinator-phase-1)]
      (println r)
      (println "-- human beverage-retail coordinator approves --")
    ...
    (println (exec-op actor "t2" {:op :log-sales-record :store-id "store-1"
                                  :patch {:units-sold 18 :item "craft ipa 6-pack" :units-returned 0}} coordinator-phase-3))
    ...
    (println (exec-op actor "t3" {:op :schedule-staffing-operation :store-id "store-1"
                                  :patch {:shift "evening-counter" :date "2026-07-20" :window "17:00-21:00"}} coordinator-phase-3))
    ...
    (println (exec-op actor "t4" {:op :coordinate-supply-order :store-id "store-1"
                                  :patch {:item "wine bottle labels" :quantity 500 :estimated-cost 85.0}} coordinator-phase-3))
    ...
    (let [r (exec-op actor "t5" {:op :coordinate-supply-order :store-id "store-1"
                                 :patch {:item "premium spirits restock" :quantity 1 :estimated-cost 3200.0}} coordinator-phase-3)]
      (println r)
      (println "-- human beverage-retail coordinator reviews & approves --")
    ...
    (let [r (exec-op actor "t6" {:op :flag-compliance-concern :store-id "store-1"
                                 :patch {:concern "customer at register 2 declined to present ID after cashier request, suspected underage purchase attempt" :confidence 0.92}} coordinator-phase-3)]
      (println r)
      (println "-- human beverage-retail coordinator reviews & approves --")
    ...
    (println (exec-op actor "t8" {:op :log-sales-record :store-id "store-3"
                                  :patch {:units-sold 10 :item "lager 12-pack"}} coordinator-phase-3))
    ...
    (let [actor-direct (op/build db {:advisor (reify advisor/Advisor
      ...
      (println (exec-op actor-direct "t9" {:op :schedule-staffing-operation :store-id "store-1"
                                           :patch {:shift "closing-counter" :date "2026-07-22"}} coordinator-phase-3)))

    (println "\n== log-sales-record store-1, advisor drifts into age-verification-override/responsible-service scope -> HARD hold, permanent ==")
