(ns beverageretailops.phase
  "Phase 0->3 staged rollout for the ISIC-4722 specialized beverage retail
  operations-coordination actor.

    Phase 0  read-only            -- no writes, still governor-gated.
    Phase 1  assisted-logging     -- sales-record logging allowed, every
    ...
    Phase 3  supervised auto      -- governor-clean, high-confidence
                                     ...
                                     auto-commit. `:flag-compliance-concern`
  ...
  rollout milestone still to come. Flagging a compliance concern (a
  suspected age-verification failure or over-service) always needs a
  human to actually look at it -- this op never itself finalizes an
  age-verification override or a responsible-service decision.
  `beverageretailops.governor`'s own `always-escalate-ops` enforces the
  same invariant independently -- two layers, not one, agree on this."
  (:require [beverageretailops.governor :as governor]))
...
(defn verdict->disposition
  "Map a BeverageRetailGovernor verdict to a base disposition before the
  phase gate."
  [verdict]
