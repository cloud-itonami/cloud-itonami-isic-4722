(ns beverageretailops.store-contract-test
  "Contract tests for `beverageretailops.store/Store` protocol."
...
(deftest mem-store-store-lookup
  (testing "MemStore can store and retrieve stores by ID (string keys)"
    (let [stores {"s1" {:store-id "s1" :name "Alice's Wine Shop" :registered? true :verified? true}}
...
(deftest mem-store-all-stores
  (testing "MemStore returns all stores in sorted order"
    (let [stores {"s2" {:store-id "s2" :name "Bob's Craft Beer Cellar"}
                  "s1" {:store-id "s1" :name "Alice's Wine Shop"}
                  "s3" {:store-id "s3" :name "Carol's Liquor Store"}}
          s (store/mem-store stores)
...
(deftest mem-store-with-stores
  (testing "MemStore with-stores replaces the store directory"
    (let [s (store/mem-store {})
          new-stores {"s1" {:store-id "s1" :name "Alice's Wine Shop"}}]
