(ns beverageretailops.governor-contract-test
  ...
  (testing "clean sales-record proposal -> auto-commit at phase 3"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t1"
                               {:op :log-sales-record :store-id "store-1" :patch {:units-sold 24 :item "cabernet sauvignon"}}
                               ctx)]
      (is (some? result))
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2"
                               {:op :flag-compliance-concern :store-id "store-1"
                                :patch {:concern "suspected underage purchase attempt" :confidence 0.99}}
                               ctx)]
      (is (some? result))
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2b"
                               {:op :coordinate-supply-order :store-id "store-1"
                                :patch {:item "premium spirits restock" :quantity 1 :estimated-cost 2400.0}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
  ...
  (testing "a low-cost :coordinate-supply-order auto-commits at phase 3 when clean"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2c"
                               {:op :coordinate-supply-order :store-id "store-1"
                                :patch {:item "wine bottle labels" :quantity 500 :estimated-cost 85.0}}
                               ctx)]
      (is (some? result))
      (is (> (count (store/coordination-log db)) 0)
...
(deftest scope-excluded-content-hard-hold
  (testing "proposal drifting into age-verification-override/responsible-service scope -> permanent hard hold"
    (let [db (store/seed-db)
          actor (op/build db)
  ...
  (testing "every decision leaves immutable audit facts"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t8a"
                     {:op :log-sales-record :store-id "store-1" :patch {:units-sold 24}}
                     ctx)
      (exec-request actor "t8b"
                     {:op :log-sales-record :store-id "unknown" :patch {:units-sold 24}}
                     ctx)
